...
- **DGX Spark 已连通，真实数据集已跑通**：`gx10-9ec6`（aarch64 / NVIDIA GB10 / CUDA 13.0）。15 张真实黄芪图经 `plant_vision` 的 `herb` 模式跑通全链路：真实视觉 → 本地语料 → 融合 → Claim-Evidence 审计，76 个观察、106 个 supported claims、0 refused。实测发现该图集全部是干燥药材切片而非植株叶片，因此视觉模式拆成 `live`（叶片）与 `herb`（药材）。**没有地面真值**，不宣称识别准确。产物 `artifacts/dgx/`，实测记录见 `phytoagent-skills/docs/dgx-spark.md`。
