...
- **DGX Spark 已连通并跑通真实视觉推理**：`gx10-9ec6`（aarch64 / NVIDIA GB10 / CUDA 13.0），`plant_vision` 新增 `live` 模式，端到端产物 `artifacts/dgx/`，实测记录见 `phytoagent-skills/docs/dgx-spark.md`。视觉输入仍是 PIL 合成图，真实叶片照片待项目所有者提供。
