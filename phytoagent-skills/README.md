...
| Agent 触发与 A/B | 每包有正向、负向、缺参数用例；真实 StepFun 评测**未运行** |
...
未实现的 `live`/`replay` 明确失败，不回退 fixture。离线部分（Demo、契约评测、测试）不联网；`harness/` 是唯一会发起网络请求的模块，必须显式配置端点与密钥，缺密钥直接报错而不是退回 fixture。私钥不随源码发布。
